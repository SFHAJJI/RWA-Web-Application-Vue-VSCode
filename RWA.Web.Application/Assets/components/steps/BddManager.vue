    ...
    <div>
        ...
        <div class="card">
            <v-container fluid v-if="!workflowStore.loading">
                <v-row>
                    <v-col cols="12" md="4">
                        <v-card @click="toggleTable('SuccessfulMatches')" class="mb-4" color="green-lighten-5"
                            elevation="2">
                    ...
                    <v-col cols="12" md="4">
                        <v-card @click="toggleTable('ToBeAddedToBDD')" class="mb-4" color="blue-lighten-5"
                            elevation="2">
                ...
                <v-expand-transition>
                    <div v-if="activeTable">
                        <v-card class="mt-4">
                            ...
                            <v-data-table :headers="headers" :items="activeData"
                                class="elevation-1 modern-table"></v-data-table>
                        </v-card>
                    </div>
                </v-expand-transition>
                <v-row>
                    <v-col>
                        <ProgressiveLoader :loading="workflowStore.stepLoading['bdd-manager']">
                            <v-btn @click="triggerUpdate" color="primary">Update BDD</v-btn>
                        </ProgressiveLoader>
                    </v-col>
                </v-row>
            </v-container>
...
const triggerUpdate = async () => {
    ...
    await workflowStore.triggerTransition('UpdateBdd');
